...
class stock_return_picking_memory(osv.osv_memory):
    ...
    _columns = {
        ...
        'prodlot_id': fields.related('move_id', 'prodlot_id', type='many2one', relation='stock.production.lot', string='Stock production lot', readonly=True),
    }
...
class stock_return_picking(osv.osv_memory):
    ...
    _columns = {
        ...
        'note': fields.text('Notes', readonly=True, states={'draft': [('readonly', False)]}),
     }
...
#        Create new picking for returned products
        ...
        new_picking = pick_obj.copy(cr, uid, pick.id, {
                                        'name': _('%s-%s-return') % (new_pick_name, (pick.name).replace(_('-return'), '')),
                                        'move_lines': [],
                                        'state':'draft',
                                        'type': new_type,
                                        'origin': _('%s-return') % (pick.origin).replace(_('-return'), ''),
                                        'date':date_cur,
