...
class tcv_bank_deposit_multi_lines(osv.osv_memory):
    ...
    _columns = {
        ...
        'multi_move_ids': fields.one2many(
            'tcv.bank.deposit.multi.lines.detail', 'multi_id', 'Details'),
    }
    ...
    def on_change_origin(self, cr, uid, ids, origin, context):
        if origin:
            ...
            multi_move_ids = obj_move.search(
                cr, uid, [('journal_id', '=', org.journal_id.id),
                          ('debit', '>', 0),
                          ('reconcile_id', '=', 0),
                          ('account_id', 'in', (
                              org.journal_id.default_debit_account_id.id,
                              org.journal_id.default_credit_account_id.id))],
                context=None)
            moves = obj_move.browse(
                cr, uid, multi_move_ids, context=None)
            lines = []
            for m in moves:
...
class tcv_bank_deposit_multi_lines_detail(osv.osv_memory):
    ...
    _columns = {
        ...
        'multi_id': fields.many2one(
            'tcv.bank.deposit.multi.lines', 'Move lines'),
        'move_id': fields.many2one(
            'account.move.line', 'Move_id', required=False),
        'move': fields.char(
            'Move', size=64, required=False, readonly=True),
        'name': fields.char(
        ...
        'selected_total': fields.float(
            'Sel amount', digits_compute=dp.get_precision('Account'),
            required=False),
        }
