...
class list_wh_iva(report_sxw. rml_parse):
    ...
    def _get_total_amount_doc(self, doc_id):
        total = 0
        for line in self.pool.get('txt.iva').browse(
                self.cr, self.uid, doc_id).txt_ids:
            total += line.invoice_id.amount_total
        return total

    def _get_alicuota(self, txt_line):
        return self.pool.get('txt.iva').get_alicuota(
            self.cr, self.uid, txt_line)

    def _get_amount_exempt(self, txt_line):
        amount_exempt, amount_untaxed = self.pool.get(
            'txt.iva').get_amount_exempt_document(self.cr, self.uid, txt_line)
    ...
    def _get_control_number(self, txt_line):
        return self.pool.get(
            'txt.iva').get_number(
                self.cr, self.uid, txt_line.invoice_id.nro_ctrl,
                'inv_ctrl', 20)

    def _get_type_document(self, line):
        return self.pool.get(
            'txt.iva').get_type_document(self.cr, self.uid, line)

    def _get_document_number(self, txt_id, txt_line):
        return self.pool.get(
            'txt.iva').get_document_number(
                self.cr, self.uid, txt_id, txt_line, 'inv_number')

    def _get_document_affected(self, line):
        return self.pool.get(
            'txt.iva').get_document_affected(self.cr, self.uid, line)

report_sxw.report_sxw(
    'report.list_wh_iva.report',
    'txt.iva',
    'tcv_fiscal_report/report/list_wh_iva.rml',
    parser=list_wh_iva,
    header=False
)
