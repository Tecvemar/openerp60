...
class parser_fiscal_book_sale_report(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context=None):
        context = context or {}
        super(parser_fiscal_book_sale_report, self).__init__(cr, uid, name, context=context)
    ...
    def _add_book_report_line(self, item, data, res):
        ...
        if item.invoice_id and item.invoice_id.dua_form_id:
            # Update export lines
            ...
            data.update({
                ...
                'custom_amount': item.vat_exempt,
                'vat_exempt_tot': 0,
                'custom_statement': item.invoice_id.dua_form_id.name,
                'base_ex': item.vat_exempt,
                })
            res.append(data)
        elif tax_lines == 1:
    ...
    def _get_lines(self, obj, adjust=False, only_total=False):
        ...
        for item in obj.fbl_ids:
            if (adjust and (item.accounting_date < min_date or item.doc_type == 'AJST')) or (
                    not adjust and (item.accounting_date >= min_date and item.doc_type != 'AJST')):
    ...
    def _get_groups(self):
        return [
            {'name': 'Ajustes a los débitos fiscales de períodos anteriores',
             'is_adjust': True,
             'tot_name': 'Sub Total ajustes'},
            {'name': 'Débitos fiscales del período',
             'is_adjust': False,
             'tot_name': 'Sub Total Débitos fiscales'},
            ]

    def _get_totals(self, obj):
        obj_fb = self.pool.get('fiscal.book')
        return obj_fb._compute_sale_book_totals(self.cr, self.uid, obj)

report_sxw.report_sxw(
    'report.tcv.fiscal.book.sale',
    'fiscal.book',
    'addons/tcv_fiscal_report/report/fiscal_book_sale_report.rml',
    parser=parser_fiscal_book_sale_report,
    header=False
    )
